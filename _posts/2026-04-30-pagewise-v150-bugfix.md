...
Chrome Extension 的版本号定义在 manifest.json 的 version 字段中。构建脚本从这个字段读取版本号来命名输出文件。如果 manifest.json 和 git tag 各自独立管理，很容易出现不同步的问题。

### Chrome Extension 事件模型

Chrome Extension 有两种监听页面变化的事件：

- tabs.onUpdated：页面加载过程中触发，changeInfo.status 为 complete 时表示加载完成
- tabs.onActivated：用户切换标签页时触发，此时新标签页的 DOM 已经加载完毕

这两个事件的触发时机不同，处理不当会导致侧边栏拿到旧数据或空数据。

### API 超时策略

不同复杂度的 API 调用需要不同的超时时间。普通问答 10-15 秒足够，但重任务（如学习路径生成，需要分析整个知识库）可能需要 45-60 秒。一刀切的硬编码超时会在重任务场景下失效。
...
修复思路——在 `onActivated` 监听器中增加内容提取调用：

**根因**：Chrome Extension 的 `tabs.onActivated` 事件只在用户切换标签页时触发，此时新标签页的 DOM 已经加载完毕，不会再触发 `onUpdated`。如果不主动调用 `extractContent()`，侧边栏拿到的就是旧数据或空数据。
...
修复——确保 `.message-ai` 使用 `flex-direction: column`：
...
30 秒的硬编码超时对这种重任务来说太短了。特别是在通过 API 网关（如 AxonHub）转发时，额外的网络延迟会进一步压缩可用时间。

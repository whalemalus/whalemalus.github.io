...
为什么这样设计？因为 Claude Code 本身已经是一个完整的 AI Agent——它能读文件、写代码、跑命令、搜索代码库。Bot 只需要把飞书的消息传给它，把它的输出传回飞书。

换句话说，Claude Code 终端里能做的事，飞书里都能做。
...
WebSocket 方案的优势是**不需要公网 IP**。Bot 主动连接飞书服务器，建立长连接后双向通信。对于部署在内网或没有域名的服务器来说，这基本是唯一靠谱的选择。

飞书的 WebSocket API 通过 `lark_oapi.ws.Client` 实现，底层是 `wss://msg-frontier.feishu.cn/ws/v2`。

### Claude Code 的 stream-json 输出

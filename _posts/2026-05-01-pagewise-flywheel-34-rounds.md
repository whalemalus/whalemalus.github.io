...
34 轮迭代，LLM Wiki 系统的全部蓝图在 24 小时内逐一实现。没有人工干预，只有 TODO.md 上一个接一个被打勾的任务。
...
Guard 管检查，Plan 管设计，Sub 管实现，各管各的。
...
当最后一个 `[ ]` 变成 `[x]`，飞轮就失去了动力。不是 Bug，是设计。
...
2. **自动化不等于无人值守**：API Key 占位符、文件权限、大文件处理，这些"小问题"在自动化环境下会变成系统性故障。

3. **测试是飞轮的轴承**：1873 个测试不是装饰，它们是让飞轮平稳转动的轴承。每一轮迭代都依赖测试来验证产出。

4. **Claude Code 是靠谱的 Sub Agent**：在权限正确、prompt 精确的条件下，Claude Code 能在一轮迭代中完成不少工作，比如 R5 顺带添加了 4 个新的内容提取方法。
